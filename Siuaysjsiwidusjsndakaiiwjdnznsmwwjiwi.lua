...
local Tab10 = Window:MakeTab({"Nick Admin", "hammer"})
...
Tab1:AddDiscordInvite({
    ...
    Logo = "rbxassetid://138397499303140",
...
local Section = Tab1:AddSection({"versao do Hub 2.2"})

local Paragraph = Tab1:AddParagraph({"Criadores", "nick "})

local Paragraph = Tab1:AddParagraph({"créditos", "Nickzin"})
  ...
  Tab1:AddButton({
    ...
    Callback = function()
      ...
      setclipboard("https://www.tiktok.com/@nickzinpcc?_t=ZM-90ZqOl4bGZP&_r=1") -- Copia o link também, se quiser só o @, remova esta linha
    end
  })
...
createMusicDropdown("Funk", {
    ["Funk"] = {
        ...
        {name = "vem no poc poc", id = "102333419023382"},
        {name = "na onda da loloi", id = "6811537974"},
        {name = "Barulho XL", id = "72182615662988"},
        {name = "", id = ""},
        {name = "", id = ""},
        {name = "", id = ""},
        {name = "", id = ""},
...
local function TornadoPirateFree()
    ...
    if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
        TextChatService.TextChannels.RBXGeneral:SendAsync(
            "[⚠️AVISO] UM TORNADO SURGIU! by Nickzin"
        )
    end
...
-- » Premium Gamepass
-- ---------------------------------------------------------
Tab11:AddDivider(" » Premium Gamepass")
...
-- Dropdown de seleção
Tab11:AddDropdown({
    Name = " Escolha sua Gamepass",
    Options = {"Gamepass Vip", "Gamepass Premium"},
    Default = nil,
    Callback = function(value)
        selectedPass = value
        print("Gamepass selecionada:", value)
    end
})

-- Botão principal para conseguir gamepass
Tab11:AddButton({"Conseguir Gamepass", function()
    if not selectedPass then
        warn("Nenhuma gamepass selecionada!")
        return
    end
    ...
    -- ID da gamepass baseado na escolha
    local GAMEPASS_ID = selectedPass == "Gamepass Vip" and 850049439 or 8021652
    ...
    gamepassLabel.Text = "Verificando " .. selectedPass .. "..."
    ...
    -- Atualizar a GUI com posição e status
    local function updateGui()
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            local pos = player.Character.HumanoidRootPart.Position
            positionLabel.Text = string.format("Posição: (%.2f, %.2f, %.2f)", pos.X, pos.Y, pos.Z)
        else
        ...
        if success and hasGamepass then
            gamepassLabel.Text = selectedPass .. ": Ativa!"
            gamepassLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
        else
            gamepassLabel.Text = selectedPass .. ": Não ativa"
    ...
    print("Free GamePass script ativado para:", selectedPass)
end})